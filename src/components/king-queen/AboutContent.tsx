import { MAJALES_DATE } from "../../config/constants";

export const AboutContent = () => {
  return (
    <div className="space-y-8 max-w-2xl text-white">
      <div>
        <h1 className="text-3xl md:text-5xl font-medium text-violet">Král a Královna Majálesu</h1>
        <p>
          Chcete zažít nezapomenutelný zážitek, soutěžit v unikátních disciplínách a získat titul Král a Královna Majálesu UTB?
          Pak neváhejte a přihlaste se společně se svým parťákem do naší jedinečné soutěže.
        </p>
      </div>
      <div>
        <h2 className="text-3xl md:text-5xl font-medium text-violet">Kdo se může stát Králem a Královnou?</h2>
        <p>
          Soutěžní dvojice musí být složena z muže a ženy, kteří studují
          na stejné fakultě UTB.<br/>
          Podrobnější informace v registračním formuláři.
        </p>
      </div>
      <div>
        <h2 className="text-3xl md:text-5xl font-medium text-violet">Kde a kdy se akce koná?</h2>
        <p>Akce se uskuteční {MAJALES_DATE.toLocaleDateString('cs-CZ').replace(/ /g, '\u00A0')} v rámci Majálesu UTB v areálu festivalu.</p>
      </div>
    </div>
  );
};
