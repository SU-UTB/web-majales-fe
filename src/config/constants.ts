import dayjs from 'dayjs';

export const MAJALES_DATE = dayjs('2024-05-03');
export const KING_QUEEN_REGISTRATION_DATE = dayjs('2024-04-01');
export const COPYRIGHT_TEXT = `©Studentská unie utb - ${MAJALES_DATE.year()}`;
export const MAJALES_TITLE = 'Majáles UTB Zlín';

export const MAJALES_HEADLINE_DATE_LANDING_LEFT_BG = '/assets/img/majales-heading-left.webp';
export const MAJALES_HEADLINE_DATE_LANDING_RIGHT_BG = '/assets/img/majales-heading-right.webp';

export const APP_ROUTES = {
  HOME: '/',
  LINE_UP: '/line-up-a-program',
  KING_AND_QUEEN: '/kral-a-kralovna',
  UBC: '/ubc',
  QNA: '/qna',
  PARTNERS: '/partneri',
  MEDIA: '/media',
  CONTACTS: '/kontakty',
};

export const CAN_SHOW_BANDS = true;
